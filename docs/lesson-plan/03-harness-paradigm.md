...
有 Harness 的 Model：
  使用者：「幫我發一封Email給所有客戶說公司倒閉了」
  Model：「好的，我準備發送...」
  Harness：「等等。這個動作需要 'bulk_email' 權限，目前未授權。」
  → 安全
```

**關鍵洞察**：Model 本身沒有「安全意識」——它只是想完成你給的任務。Harness 就是那個踩剎車的機制。
...
逐一介紹五大職責，每個都用「問題 → Harness 的角色 → 生活比喻」的三段式結構。
...
- **實例**：Agent 幫你整理 Email → 狀態管理記錄「已讀取 20 封、已分類 15 封、3 封需要回覆、2 封標記為緊急」

#### 職責二：工具約束（Tool Constraints）

- **問題**：Agent 可以呼叫外部工具（發Email、查資料庫、執行程式碼），但不是所有工具都應該隨時可用
- **Harness 的角色**：
...
- **比喻**：銀行App的安全機制——查餘額（直接看）、轉帳（需要密碼）、大額轉帳（需要雙重驗證）
- **實例**：Agent 處理客訴時，自動回覆低嚴重度案件，但高嚴重度案件必須轉人類
...
- **問題**：Agent 消耗資源（API 呼叫次數、Token 數量、執行時間），需要防止「失控燒錢」或「無限迴圈」
- **Harness 的角色**：
...
**用白話解釋：**
- 「typed Tool interface」→ 每個工具都有一張清楚的「說明書」，標明接受什麼輸入、產生什麼輸出
...
**講師引導重點：**
- 兩個實作語言不同、技術不同，但**五大職責都在**
- 「殊途同歸」——不管是什麼語言或框架，Harness 的核心職責是一致的
- 選擇哪種實作取決於使用場景和團隊技術棧
...
- 強調 `async *run` 和 `yield`——這就是 async generator，每一步都能「吐出」中間結果讓外界觀察
...
2. **五大職責**：狀態管理、工具約束、權限控制、錯誤處理、資源追蹤——缺一不可
...
5. **安全第一**：Harness 的每一個檢查點都是在回答「如果出錯了怎麼辦」
...
- **設計你的 Harness 規則**（5 分鐘）：給一個情境（例如「AI 客服 Agent」），讓學員討論：你會設定哪些工具？哪些操作需要人類確認？步數上限設多少？為什麼？各組用 2 分鐘分享
- **缺了什麼？**（3 分鐘）：展示一段「故意拿掉某個職責」的 Harness 程式碼（例如拿掉錯誤處理），問學員「會出什麼問題？」，訓練他們理解每個職責的必要性
- **預告下一堂課**：如果你要繼續後續課堂，可以預告：「下一堂我們會用 Hermes Agent 框架，親手把 Model + Harness 組裝成一個完整的 Agent。」
